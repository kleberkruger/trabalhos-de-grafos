...
double Graph::mst(const Graph &graph, Graph &mst, int version) {
    ...
    static MSTAlgorithm algorithms[] = {
            ...
            MSTAlgorithm("Prim with FibonacciHeap", 3, Graph::prim<BinaryHeap>),
    };

    std::cout << "Running: \"" << algorithms[version].name << "\" version " << algorithms[version].version << std::endl;
    return algorithms[version].execute(graph, mst);
}
